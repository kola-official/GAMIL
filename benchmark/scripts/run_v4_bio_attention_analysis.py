#!/usr/bin/env python3
"""Resumable biological attention analysis for Realm-Rank v4 test records."""
...
REALM_RANK_OUTPUT_ROOT = Path(
    os.environ.get("OUTPUT_ROOT", CHECKPOINT_ROOT / "realm_rank_v4_six_model")
).resolve()
PAPER_CODE = Path(
...
TEST_FASTA = PROCESSED_DATA_ROOT / "realm_rank_v4" / "test.fasta.gz"
PRED_ROOT = REALM_RANK_OUTPUT_ROOT / "benchmark" / "test_predictions"
MP_PRED = PRED_ROOT / "viralm_r_v4_final.csv"
GA_PRED = PRED_ROOT / "viralm_r_v4_final_12l_gated_mil.csv"
GA_MODEL_DIR = REALM_RANK_OUTPUT_ROOT / "models" / "viralm_r_v4_final_12l_gated_mil"
DEFAULT_OUT = REALM_RANK_OUTPUT_ROOT / "bio_attention_v4_test"
...
def build_case_records(out_dir: Path) -> None:
    ...
    log_message(out_dir, "building case groups from fixed v4 test predictions")
...
def select_fasta_records(out_dir: Path) -> None:
    case_path = out_dir / "case_records.tsv"
    fasta_path = out_dir / "selected_v4_records.fna"
    checksum_path = out_dir / "selected_v4_records.checksums.tsv"
    if stage_done(out_dir, "selected_fasta", [fasta_path, checksum_path]):
        log_message(out_dir, "stage selected_fasta already done")
        return

    log_message(out_dir, "selecting exact FASTA blocks from realm_rank_v4/test.fasta.gz")
...
def run_attention_worker(args: argparse.Namespace) -> None:
    ...
    from benchmark_realm_rank_v4_test import GenomeCollator
...
def run_genomad(out_dir: Path, args: argparse.Namespace, genomad_db: Path) -> None:
    ...
    if stage_done(out_dir, "genomad", [features_path]):
        ...
        return
    log_message(out_dir, "running geNomad end-to-end on selected v4 records")
    cmd = [
        ...
        str(out_dir / "selected_v4_records.fna"),
        str(genomad_out),
        str(genomad_db),
    ]
    try:
...
def parse_genomad_features(out_dir: Path, genomad_out: Path, features_path: Path) -> None:
    ...
    for path in all_tsv:
        ...
        if not rows:
            continue
        if lower.endswith("_features.tsv") or lower == "selected_v4_records_features.tsv":
            for row in rows:
...
def select_checkv_fasta(out_dir: Path) -> Path:
    ...
    for row in rows:
        if safe_int(row["binary_label"]) == 1:
            ...
            selected.add(row["record_id"])
    out = out_dir / "checkv_selected_v4_records.fna"
    with open(out, "w") as handle:
        for record_id, block in iter_fasta_blocks(out_dir / "selected_v4_records.fna"):
...
def verify_outputs(out_dir: Path) -> None:
    ...
    checksum_rows = read_tsv(out_dir / "selected_v4_records.checksums.tsv")
    add(
