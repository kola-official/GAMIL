# Realm-Rank test 生物学 attention 分析方法与结果解读
...
| 简称 | 模型 | 直观理解 |
|---|---|---|
| MP | `viralm_r` | 原始模型/基线模型。它给每个 fragment 和整个 genome 一个病毒概率。 |
| GA | `viralm_r_12l_gated_mil` | 带 gated attention 的 MIL 模型。它不仅给概率，还会给同一个 genome 内每个 fragment 一个 attention weight。 |
...
`processed_data/realm_rank/test.fasta.gz`

关键点：

- 没有重新切割序列。
- 没有扩展窗口。
- 没有使用 `realm_rank_benchmark/bench-*`。
- 从原始 Realm-Rank test FASTA 中按 `record_id` 直接筛选目标片段。
...
| selected FASTA checksum 检查 | 3300/3300 全部与原始 Realm-Rank test 记录一致 |

所以后续 geNomad/CheckV 看到的是原始 Realm-Rank test 片段本身，不是重新加工后的序列。
...
本分析中 CheckV 不是主统计依据，只是辅助增强，因为 test records 多数是短 fragment，CheckV 对短片段通常会给低质量或无法判断。
...
| `selected_records.fna` | 从原始 Realm-Rank test FASTA 直接筛选出的序列，用于 geNomad/CheckV。 |
| `selected_records.checksums.tsv` | 验证 selected FASTA 是否与原始 Realm-Rank test 完全一致。 |
