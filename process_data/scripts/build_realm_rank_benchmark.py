#!/usr/bin/env python3
"""Build fixed-length Realm-Rank benchmark files.

This script keeps the existing Realm-Rank train/dev/test outputs intact and
creates a separate fixed-length benchmark suite under
``processed_data/realm_rank_benchmark``.
"""
...
def log(message: str) -> None:
    print(f"[realm-rank-benchmark] {message}", flush=True)
...
def prepare_references(
    ...
    if not (virus_db.with_suffix(".nhr").exists() or (virus_db.parent / "virus.nal").exists()):
        log("building virus BLAST database for reference decontamination")
...
def apply_leakage_filter(
    ...
    removed_tsv = metadata_dir / "test_removed_by_train_dev_blast.tsv"
...
def write_verification_qc(
    ...
    removed_path = metadata_dir / "test_removed_by_train_dev_blast.tsv"
...
def write_summary(
    ...
    lines = [
        "# Realm-Rank Benchmark Summary",
        "",
        f"Generated at: {datetime.now().isoformat(timespec='seconds')}",
        "",
        "The existing `processed_data/realm_rank/test.fasta.gz` is retained as the 300-2000 bp held-out test set. "
        "This output is a separate fixed-length benchmark suite.",
        "",
        "## References",
        "",
    ]
    if include_references:
        ...
        lines.extend(
            [
                ...
                "Insect negatives come from the Realm-Rank test split.",
            ]
        )
    else:
    ...
    if run_train_blast:
        lines.append(
            f"- Benchmark-vs-train+dev leakage: final `processed_data/realm_rank/train.fasta.gz` and `dev.fasta.gz` were decompressed and indexed; "
    ...
    if run_train_blast:
        lines.append("- `metadata/test_removed_by_train_dev_blast.tsv` records candidates removed by final-train+dev BLAST coverage.")
    else:
        lines.append("- `metadata/test_removed_by_train_dev_blast.tsv` is header-only because candidate-vs-train+dev BLAST was skipped.")
    lines.append(
    ...
    (output_dir / "data_process_test_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--realm-rank-dir", type=Path, default=PROCESSED_DATA_ROOT / "realm_rank")
    parser.add_argument("--output-dir", type=Path, default=PROCESSED_DATA_ROOT / "realm_rank_benchmark")
    ...
    parser.add_argument("--force", action="store_true", help="Remove and rebuild the output directory.")
