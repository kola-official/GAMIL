#!/usr/bin/env python3
"""Build Realm-Rank train/dev/test FASTA files.

Realm-Rank uses a fixed public-source policy and decontamination flow, with the
model-selection split defined at dataset-build time:
...
def ensure_dirs(output_dir: Path, force: bool) -> tuple[Path, Path, Path, Path]:
    ...
    if output_dir.exists() and any((output_dir / name).exists() for name in ("train.fasta.gz", "dev.fasta.gz", "test.fasta.gz")) and not force:
        raise RuntimeError(f"{output_dir} already contains output; use --force to rebuild")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-dir", type=Path, default=PROCESSED_DATA_ROOT / "realm_rank")
    ...
    parser.add_argument(
        "--reuse-decontam-from",
        type=Path,
        help="Reuse split-independent nonviral-vs-virus BLAST decontamination outputs from this Realm-Rank directory.",
    )
