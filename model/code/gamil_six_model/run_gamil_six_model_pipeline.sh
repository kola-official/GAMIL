...
OUTPUT_ROOT="${OUTPUT_ROOT:-${CHECKPOINT_ROOT}/gamil_six_model}"
MODEL_ROOT="${OUTPUT_ROOT}/models"
INIT_ROOT="${OUTPUT_ROOT}/init"
LOG_ROOT="${OUTPUT_ROOT}/logs"
STATE_ROOT="${OUTPUT_ROOT}/state"
BENCHMARK_ROOT="${OUTPUT_ROOT}/benchmark"
DATA_PATH="${DATA_PATH:-${PROCESSED_DATA_ROOT}/realm_rank/train_csv}"
TEST_FASTA="${TEST_FASTA:-${PROCESSED_DATA_ROOT}/realm_rank/test.fasta.gz}"
...
STUDENT_R_6L="${STUDENT_R_6L:-${INIT_ROOT}/viralm_r_6l}"
...
TMUX_SESSION="${TMUX_SESSION:-gamil_six_model}"
...
if [[ "${RUN_IN_TMUX}" == "1" && "${REALM_RANK_V4_SIX_MODEL_WORKER:-0}" != "1" ]]; then
  ...
  printf -v CMD_Q '%q ' env \
    ...
    bash "${SCRIPT_DIR}/run_gamil_six_model_tmux.sh"
  tmux new-session -d -s "${TMUX_SESSION}" "cd ${SCRIPT_DIR_Q} && ${CMD_Q}"
  echo "Started Realm-Rank six-model pipeline in tmux."
...
echo "[$(timestamp)] Realm-Rank six-model pipeline"
...
run_torch_logged viralm_r_6l_meanpool_kd "$((BASE_MASTER_PORT + 2))" "${SCRIPT_DIR}/train_meanpool_kd.py" \
  --student-model "${STUDENT_R_6L}" \
  --teacher-model "${TEACHER_R}" \
  --output-dir "${MODEL_ROOT}/viralm_r_6l_meanpool_kd" \
  --run-name viralm_r_6l_meanpool_kd \
  --per-device-train-batch-size "${MEANPOOL_BATCH_SIZE}" \
  --per-device-eval-batch-size "${MEANPOOL_EVAL_BATCH_SIZE}" \
  --dataloader-num-workers "${MEANPOOL_DATALOADER_WORKERS}" \
  "${COMMON_CACHE_ARGS[@]}"
...
run_torch_logged viralm_r_6l_gated_mil_kd "$((BASE_MASTER_PORT + 4))" "${SCRIPT_DIR}/train_gated_mil_kd.py" \
  --backbone-model "${MODEL_ROOT}/viralm_r_6l_meanpool_kd" \
  --teacher-model "${TEACHER_R}" \
  --resume-mil-state "${MODEL_ROOT}/viralm_r_6l_gated_mil_kd/best_mil_model.pt" \
  --output-dir "${MODEL_ROOT}/viralm_r_6l_gated_mil_kd" \
  --run-name viralm_r_6l_gated_mil_kd \
  --batch-size "${MIL_BATCH_SIZE}" \
  --eval-batch-size "${MIL_EVAL_BATCH_SIZE}" \
  --grad-accum-steps "${MIL_GRAD_ACCUM_STEPS}" \
  --scan-chunk "${SCAN_CHUNK}" \
...
run_torch_logged viralm_r_12l_gated_mil "$((BASE_MASTER_PORT + 6))" "${SCRIPT_DIR}/train_gated_mil_supervised.py" \
  --backbone-model "${TEACHER_R}" \
  --output-dir "${MODEL_ROOT}/viralm_r_12l_gated_mil" \
  --run-name viralm_r_12l_gated_mil \
  --batch-size "${MIL_BATCH_SIZE}" \
  --eval-batch-size "${MIL_EVAL_BATCH_SIZE}" \
  --grad-accum-steps "${MIL_GRAD_ACCUM_STEPS}" \
  --scan-chunk "${MIL_12L_SCAN_CHUNK}" \
...
BENCHMARK_ARGS=(
  "${SCRIPT_DIR}/benchmark_realm_rank_test.py"
